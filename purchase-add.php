...
<body class="bg-gray-100">
    ...
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-4 bg-white border border-gray-200 rounded-md shadow-md text-center">
        <form action="inc/purchase-add.inc.php" method="post">
            ...
            <select class="mb-4 border border-gray-200 rounded bg-gray-100 p-2 w-full" name="supplier" required>
                <?php
                    echo '<option value="" style="display:none">Supplier</option>';
                    include 'inc/connect.inc.php';
                    $query = "SELECT * FROM suppliers ORDER BY supplier_name ASC";
                    $result = mysqli_query($connect, $query);
                    while($row = mysqli_fetch_assoc($result)){
                        echo '<option value="'.$row['supplier_id'].'">'.$row['supplier_name'].' ('.$row['supplier_contact'].')</option>';
                    }
